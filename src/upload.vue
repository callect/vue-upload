...
export default {
  ...
  watch: {
    ...
    auto() {
      if (this.auto) {
        this.uploadFiles()
        this.$emit('resetAuto', false)
      }
    }
  },
  mounted () {
    if (this.crop) {
      this.$emit('resetMultiple', false)
      this.$emit('resetExt', 'jpg')
      this.$emit('resetAuto', false)
    }
  ...
  methods: {
    fileChange(e) {
      ...
      if (this.crop) {
        ...
        fr.onload = e => {
          ...
          this.img.onload = () => {
            this.height = this.img.naturalHeight / (this.img.naturalWidth / this.width)
            let fullScreenHeight = window.screen.height - 50
            if (this.height > fullScreenHeight) {
              this.thumbWidth = this.width
              this.thumbHeight = fullScreenHeight
            } else {
              this.thumbWidth = this.width
            ...
            this.canvas.setAttribute('style', 'position:absolute;left:0;top:0;opacity:0;z-index:-1')
              ...
              if (xhr.readyState == 4) {
                ...
                if (xhr.status == 200) {
                  ...
                  if (data.result != undefined) {
                    this.$emit('newImage', data.result)
                  }
    ...
    cropToCanvas() {
      ...
      // console.log(sw, sy, sw, sh, this.cropGetRatio())
      this.context.drawImage(this.img, sx, sy, sw, sh, 0, 0, this.width * 2, this.height * 2)
    },
    cropZoom() {
    ...
    cropOK() {
      ...
      if (this.server == '' || this.api == '') {
        this.$emit('cropImage', {image: data, width: this.width * 2, height: this.height * 2})
        this.showCrop = false
...
<template>
  <div class="upload">
    ...
    <div class="crop" v-if="showCrop" :style="{height:cropHeight+'px'}">
      ...
      <div class="dialog" :style="{width:width+'px',top:cropTop+'px'}" v-if="showCropDialog">
        <section :style="{width:thumbWidth+'px',height:thumbHeight+'px'}" :class="{'grab':grabbing==false,'grabbing':grabbing}">
